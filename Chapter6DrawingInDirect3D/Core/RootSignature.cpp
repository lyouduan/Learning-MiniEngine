...
void RootSignature::Finalize(const std::wstring& name, D3D12_ROOT_SIGNATURE_FLAGS Flags)
{
    ...
    for (UINT Param = 0; Param < m_NumParameters; ++Param)
    {
        ...
        if (RootParam.ParameterType == D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE)
        {
            ...
            // we keep track of sampler descriptor tables separately from CBV_SRV_UAV decriptor tables
    ...
    // find the rootsignature in hashtable first which saved the compiled code. 
    ...
    bool firstCompile = false;
    {
        ...
        // reserve space so that next inquiry will find that someone got here first
    ...
    if (firstCompile)
    {
        ...
        ASSERT_SUCCEEDED(g_Device->CreateRootSignature(1, pOutBlob->GetBufferPointer(), pOutBlob->GetBufferSize(),
            MY_IID_PPV_ARGS(&m_RootSignature)));

        m_RootSignature->SetName(name.c_str());

        s_RootSignatureHashMap[HashCode].Attach(m_RootSignature);
