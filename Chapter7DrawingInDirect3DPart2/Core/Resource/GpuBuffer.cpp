	...
	const void* initialData)
{
	...
	HeapProps.Type = D3D12_HEAP_TYPE_DEFAULT; // default heap (GPU visible)
	...
	// if initial data is not null
	if (initialData)
		CommandContext::InitializeBuffer(*this, initialData, m_BufferSize);

	// create views for bounding pipeline
	CreateDerivedViews();
}
	...
	const UploadBuffer& srcData, uint32_t srcOffset)
{
	...
	// use upload heap to transfer data to GPU
