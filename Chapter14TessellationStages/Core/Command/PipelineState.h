...
public:
	...
	void SetGeometryShader(const Microsoft::WRL::ComPtr<ID3DBlob>& Binary) {
		m_PSODesc.GS = CD3DX12_SHADER_BYTECODE(static_cast<void*>(Binary->GetBufferPointer()), Binary->GetBufferSize());
	}
	void SetHullShader(const Microsoft::WRL::ComPtr<ID3DBlob>& Binary) {
		m_PSODesc.HS = CD3DX12_SHADER_BYTECODE(static_cast<void*>(Binary->GetBufferPointer()), Binary->GetBufferSize());
	}
	void SetDomainShader(const Microsoft::WRL::ComPtr<ID3DBlob>& Binary) {
		m_PSODesc.DS = CD3DX12_SHADER_BYTECODE(static_cast<void*>(Binary->GetBufferPointer()), Binary->GetBufferSize());
	}
...
public:
	ComputePSO(const wchar_t* Name = L"Unnamed ComputePSO PSO");
