...
// this is an unbounded resource descriptor allocator. It is intended to provide space for 
// CPU-visible resource descriptors as resources are created. For these that need to be
// made shader-visible, they will need to be copied to a DescriptorHeap or a DynamicDescripotrHeap.
...
protected:
	...
	static const uint32_t sm_NumDescritporsPerHeap = 256;
...
public:
	...
	DescriptorHandle operator[] (uint32_t arrayIdx) const { return m_FristHandle + arrayIdx * m_DescriptorSize; }
	uint32_t GetOffsetOfHandle(const DescriptorHandle& DHandle)
	{
		return (uint32_t)(DHandle.GetCpuPtr() - m_FristHandle.GetCpuPtr()) / m_DescriptorSize;
	}
...
protected:
	...
	DescriptorHandle m_FristHandle;
