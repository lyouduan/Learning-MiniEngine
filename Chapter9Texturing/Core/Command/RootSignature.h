...
public:
    ...
    // root constants
    void InitAsConstants(UINT Register, UINT NumDwords, D3D12_SHADER_VISIBILITY Visibility = 
        D3D12_SHADER_VISIBILITY_ALL, UINT Space = 0)
    {
        ...
        m_RootParam.Constants.RegisterSpace = Register;
        m_RootParam.Constants.ShaderRegister = Space;
    }

    // CBV
    void InitAsConstantBuffer(UINT Register, D3D12_SHADER_VISIBILITY Visibility =
        D3D12_SHADER_VISIBILITY_ALL, UINT Space = 0)
    {
...
public:
    ...
    {
        if (NumRootParams > 0)
            // reset : delete current pointers if had, set new opinters
    ...
    // find in hash map first, if not exist, then compile the root signature
