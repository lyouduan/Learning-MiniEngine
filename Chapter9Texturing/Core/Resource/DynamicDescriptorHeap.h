...
// It inernally caches CPU descriptors so that when not enough space is available in the current heap,
// necessary descriptors can be re-copied to the new heap.
// 当前堆不足时，动态生成描述符堆，并把相应内容复制到新堆
class DynamicDescriptorHeap
{
public:
	DynamicDescriptorHeap(CommandContext& Context, D3D12_DESCRIPTOR_HEAP_TYPE HeapType);
...
private:
	...
	// Describes a descriptor table entry :
